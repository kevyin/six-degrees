...
void listMovies(const string& player, const vector<film>& credits)
{
...
void listCostars(const string &player, const vector<film>& credits, const imdb& db)
{
...
void listAllMoviesAndCostars(const string& player,
				    const imdb& db)
{
  vector<film> credits;
...
void queryForActors(const imdb& db)
{
  while (true) {
...
{
  if (argc != 2) {
    cerr << "Usage: imdb-test <data-files-path>" << endl;
    return 1;
  }
